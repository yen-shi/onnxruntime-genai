...
def main(args):
    ...
    while True:
        ...
        if args.timings:
            ...
            print(
                f"Prompt length: {input_tokens}, New tokens: {len(new_tokens)}, Total tokens: {total_tokens}, Time to first: {(prompt_time):.2f}s, Prompt tokens per second: {input_tokens / prompt_time:.2f} tps, New tokens per second: {len(new_tokens) / run_time:.2f} tps"
            )
